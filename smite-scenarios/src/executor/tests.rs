...
fn execute_recv_commitment_signed_after_sending_on_the_temporary_id_is_ignored() {
    ...
    executor
        .execute(
            &Program {
                instructions: v2_flow_instructions(vec![
                    // v27 is the temporary_channel_id: an id the negotiation
                    // resolves, but not the one the peer answers on.
                    Instruction {
                        operation: Operation::SendCommitmentSigned,
                        inputs: vec![36, 10, 27],
                    }, // v37
        ...
        .expect("a commitment_signed we never sent on this id is not a target bug");
    ...
    assert!(
        executor.channel_states.is_empty(),
